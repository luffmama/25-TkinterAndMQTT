...
def tkinter_stuff(client):
    ...
    right_speed_entry.grid(row=0,column=1)

    #s = [left_speed_entry,right_speed_entry]
    #s = [left_speed_entry.get(),right_speed_entry.get()]

    forward_button = ttk.Button(main_frame, text="Forward")
    forward_button.grid(row=1,column=0)
    forward_button['command'] = lambda: client.send_message("forward", [600,600])
    root.bind('<Up>', lambda event: print("Forward key"))

    #client.send_message("forward", [left_speed_entry.get(), right_speed_entry.get()])
    root.mainloop()
    ...
    right_button['command'] = lambda: print(right_speed_entry)
    ...
    down_button.grid(row=1,column=6)
